...
*/  
        ...
        const coveredDistance = this.speed * deltaT * 1.5;
...
*/
        if (centerIncrement.x < 0) { // The ball is moving to the left
            if (this.center.x <= this.player1.center.x + this.player1.halfSize.x + this.radius &&
                this.center.y <= this.radius + this.player1.center.y + this.player1.halfSize.y &&
                this.center.y >= -this.radius + this.player1.center.y - this.player1.halfSize.y &&
                this.center.x >= this.player1.center.x - this.player1.halfSize.x - this.radius) { // The ball hit player 1 racket
                this.direction = Math.PI - this.direction; // The ball rebounds
                if(this.player1.keyStates.right && this.center.x < 0){
                    this.speed*=1.15;
                }
                if(this.player1.keyStates.left && this.center.x < 0){
                    this.speed/=1.25;
                }
                if(this.player1.keyStates.down && this.center.x <0){
                    this.spin=1;
                }
                if(this.player1.keyStates.up && this.center.x <0){
                    this.spin=2;
                }
            }
        }
        ...
        /* To-do #12 - Check if the ball hit player 2 racket
            ...
            if (this.center.x <= this.player2.center.x + this.player2.halfSize.x + this.radius &&
                this.center.y <= this.radius + this.player2.center.y + this.player2.halfSize.y &&
                this.center.y >= -this.radius + this.player2.center.y - this.player2.halfSize.y &&
                this.center.x >= this.player2.center.x - this.player2.halfSize.x - this.radius) { // The ball hit player 1 racket
                this.direction = Math.PI - this.direction; // The ball rebounds
                if(this.player2.keyStates.right && this.center.x > 0){
                    this.speed/=1.25;
                }
                if(this.player2.keyStates.left && this.center.x > 0){
                    this.speed*=1.15;
                }
                if(this.player2.keyStates.down && this.center.x >0){
                   this.spin=1;
                }
                if(this.player2.keyStates.up && this.center.x >0){
                   this.spin=2;
                }
            }
        }
...
*/
        if (this.center.y - this.radius <= -this.table.halfSize.y && centerIncrement.y < 0 || // The ball is moving down and hit the bottom line
            this.center.y + this.radius >= this.table.halfSize.y && centerIncrement.y > 0) { // The ball is moving up and hit the top line
