...
export default class Table extends THREE.Group {
    constructor(parameters) {
       ...
       /*  To-do #1 - Create the net (a dashed line segment) with properties defined by the following parameters:
        ...
        let point=[];
        point.push(new THREE.Vector2(0.0,-this.halfSize.y));    
        point.push(new THREE.Vector2(0.0,this.halfSize.y));    
        geometry = new THREE.BufferGeometry().setFromPoints(point);
        material = new THREE.LineDashedMaterial( {color: this.color,linewidth: 1,scale: 1,dashSize:  this.size.y / (2.0 * this.dashes - 1.0),gapSize:  this.size.y / (2.0 * this.dashes - 1.0)} );
